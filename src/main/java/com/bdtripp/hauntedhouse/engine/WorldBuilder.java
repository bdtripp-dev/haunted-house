...
public class WorldBuilder {
    ...
    private Map<RoomName, Room> createRooms() {
        ...
        rooms.put(RoomName.ROOT_CELLAR, new Room(RoomName.ROOT_CELLAR, "in the root cellar"));
        rooms.put(RoomName.LIBRARY, new Room(RoomName.LIBRARY, "in the library"));
        rooms.put(RoomName.BILLIARD_ROOM, new Room(RoomName.BILLIARD_ROOM, "in the billiard room"));
        rooms.put(RoomName.DEN, new Room(RoomName.DEN, "in the musty den"));
        rooms.put(RoomName.WINE_CELLAR, new Room(RoomName.WINE_CELLAR, "in the wine cellar"));
        rooms.put(RoomName.BATHROOM, new Room(RoomName.BATHROOM, "in a flooded bathroom...gross"));
    ...
    private Map<String, Item> createReferencedItems() {
        ...
        items.put("key", new Item("key", "a rusty skeleton key", 1, false, PlayerStat.NONE, 0));
        items.put("potion", new Item("potion", "a powerful muscle building potion", 50, true,
                PlayerStat.MAX_CARRY_WEIGHT, 50));
    ...
    /**
     * Create and place the unreferenced items into rooms. Unreferenced items are created and placed
     * in the room all within this method because they do not need to be refered to later.
     * 
     * @param rooms the rooms to place items in
     */
    private void placeUnreferencedItems(Map<RoomName, Room> rooms) {
        rooms.get(RoomName.HALLWAY)
                .addItem(new Item("elixir", "an elixir", 50, true, PlayerStat.HEALTH, 10));
        rooms.get(RoomName.HALLWAY)
                .addItem(new Item("cookie", "a magic cookie", 5, true, PlayerStat.STRENGTH, 5));
        rooms.get(RoomName.INDOOR_GARDEN)
                .addItem(new Item("plant", "fox glove", 5, false, PlayerStat.NONE, 0));
        rooms.get(RoomName.WINE_CELLAR)
                .addItem(new Item("crate", "a big old crate", 2000, false, PlayerStat.NONE, 0));
        rooms.get(RoomName.BATHROOM)
                .addItem(new Item("bucket", "an empty bucket", 20, false, PlayerStat.NONE, 0));
    }
    ...
    /**
     * Connect the rooms together so that a player can move from room to room at specific locations
     * 
    ...
    private void wireExits(Map<RoomName, Room> rooms) {
        ...
        rooms.get(RoomName.INDOOR_GARDEN).setExit(Direction.EAST, rooms.get(RoomName.BILLIARD_ROOM),
                ExitType.UNLOCKED);
        ...
        rooms.get(RoomName.ROOT_CELLAR).setExit(Direction.EAST, rooms.get(RoomName.LIBRARY),
                ExitType.UNLOCKED);
        rooms.get(RoomName.ROOT_CELLAR).setExit(Direction.SOUTH, rooms.get(RoomName.WINE_CELLAR),
                ExitType.UNLOCKED);
        rooms.get(RoomName.ROOT_CELLAR).setExit(Direction.WEST, rooms.get(RoomName.DEN),
                ExitType.UNLOCKED);

        rooms.get(RoomName.LIBRARY).setExit(Direction.NORTH, rooms.get(RoomName.BILLIARD_ROOM),
                ExitType.UNLOCKED);
        rooms.get(RoomName.LIBRARY).setExit(Direction.WEST, rooms.get(RoomName.ROOT_CELLAR),
                ExitType.UNLOCKED);

        rooms.get(RoomName.BILLIARD_ROOM).setExit(Direction.SOUTH, rooms.get(RoomName.LIBRARY),
                ExitType.UNLOCKED);
        rooms.get(RoomName.BILLIARD_ROOM).setExit(Direction.WEST, rooms.get(RoomName.INDOOR_GARDEN),
                ExitType.UNLOCKED);
        ...
        rooms.get(RoomName.WINE_CELLAR).setExit(Direction.NORTH, rooms.get(RoomName.ROOT_CELLAR),
                ExitType.UNLOCKED);
        rooms.get(RoomName.WINE_CELLAR).setExit(Direction.EAST, rooms.get(RoomName.BATHROOM),
                ExitType.UNLOCKED);

        rooms.get(RoomName.BATHROOM).setExit(Direction.WEST, rooms.get(RoomName.WINE_CELLAR),
                ExitType.UNLOCKED);
