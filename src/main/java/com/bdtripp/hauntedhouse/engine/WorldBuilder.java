...
public class WorldBuilder {
    ...
    private Map<RoomName, Room> createRooms() {
        ...
        rooms.put(RoomName.INDOOR_GARDEN, new Room(RoomName.INDOOR_GARDEN, "in a misty indoor garden"));
        ...
        rooms.put(RoomName.OUTSIDE, new Room(
                RoomName.OUTSIDE, "outside of the haunted house and the sun is so delightful!"));
    ...
    /**
     * Create and place the unreferenced items into rooms. Unreferenced
     * items are created and placed in the room all within this method
     * because they do not need to be refered to later.
     * 
    ...
    public void placeUnreferencedItems(Map<RoomName, Room> rooms) {
        ...
        rooms.get(RoomName.HALLWAY).addItem(new Item(
                ...
                PlayerStat.STRENGTH,
                5));
        rooms.get(RoomName.INDOOR_GARDEN).addItem(new Item(
                "plant",
                "fox glove",
                5,
                false,
                PlayerStat.NONE,
                0));
        rooms.get(RoomName.WINE_CELLAR).addItem(new Item(
                "crate",
                "a big old crate",
                2000,
                false,
                PlayerStat.NONE,
                0));
        rooms.get(RoomName.BATHROOM).addItem(new Item(
                "bucket",
                "an empty bucket",
                20,
                false,
    ...
    /**
     * Connect the rooms together so that a player can move from room to room
    ...
    public void wireExits(Map<RoomName, Room> rooms) {
        ...
        rooms.get(RoomName.ROOT_CELLAR).setExit(Direction.SOUTH, rooms.get(RoomName.WINE_CELLAR),
                ExitType.UNLOCKED);
        rooms.get(RoomName.ROOT_CELLAR).setExit(Direction.WEST, rooms.get(RoomName.DEN), ExitType.UNLOCKED);
        ...
        rooms.get(RoomName.DEN).setExit(Direction.EAST, rooms.get(RoomName.ROOT_CELLAR), ExitType.UNLOCKED);
        rooms.get(RoomName.DEN).setExit(Direction.SOUTH, rooms.get(RoomName.HALLWAY), ExitType.UNLOCKED);
