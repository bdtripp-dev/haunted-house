document.addEventListener('DOMContentLoaded', async () => {
  ...
  function updateCursor() {
    ...
    const rectIsInvalid =
      (rect.left === 0 && rect.top === 0 && state.buffer.length > 0) || rect.height === 0;
  ...
  dom.input.addEventListener('keydown', async (e) => {
    if (e.key === 'Backspace' && state.cursorPosition !== 0) {
      e.preventDefault();
      state.buffer =
        state.buffer.slice(0, state.cursorPosition - 1) + state.buffer.slice(state.cursorPosition);
      ...
      e.preventDefault();
      state.buffer =
        state.buffer.slice(0, state.cursorPosition) + state.buffer.slice(state.cursorPosition + 1);
      state.userMovedCaret = false;
    } else if (e.key === 'Tab') {
      e.preventDefault();
      return;
    ...
    debug('Buffer length:', state.buffer.length);
    debug('Cursor position:', state.cursorPosition);
  });
